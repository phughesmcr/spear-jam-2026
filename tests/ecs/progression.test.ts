...
import { PlayerEquipment, PlayerInventory, PlayerProgress, PlayerTurnEffects } from "@/src/ecs/components.ts";
import {
...
Deno.test("player progression defaults to melee with empty ECS resources", async () => {
  ...
  writePlayerStateToComponents(world, player, createPlayerState());

  assertEquals(playerStateFor(world, player), {
    heldKeys: [],
    selectedWeapon: 1,
    unlockedWeapons: [1],
...
Deno.test("player progression starts from a normalized selected weapon", async () => {
  ...
  assertEquals(playerStateFor(world, player).unlockedWeapons, [1, 2]);
});
...
Deno.test("player progression tracks weapons and ammo in ECS components", async () => {
  ...
  assertEquals(playerStateFor(world, player).unlockedWeapons, [1, 2, 3]);
...
Deno.test("player progression ticks active turn effects in ECS components", async () => {
  ...
  assertEquals(playerStateFor(world, player).turnEffects, [
...
Deno.test("player progression returns credit and XP events from ECS progress", async () => {
  ...
  assertEquals(playerStateFor(world, player).progress, { credits: 15, score: 17, xp: 24, levelCredits: 0 });
});
...
Deno.test("player progression clears transient key and uplink ECS state", async () => {
  ...
  assertEquals(playerStateFor(world, player).heldKeys, []);
  assertEquals(playerStateFor(world, player).hasUplinkCode, false);
  assertEquals(world.components.getEntityData(PlayerInventory, player), {
    keyMask: 0,
    hasUplinkCode: 0,
    pistolAmmo: 0,
