...
Deno.test("game audio presentation resolves content ids and grid poses into neutral audio operations", () => {
  const sound = soundCatalogEntry(SoundId.PickupItem);
  assertEquals(audioCueFor({ soundId: SoundId.PickupItem, position: { x: 3, y: 7 }, radius: 4, volume: 0.5 }), {
  ...
  assertEquals(audioEmitterFor({ entity, soundId: SoundId.AmbientHum, x: 2, y: 4, radius: 6, volume: 0.75 }), {
    id: entity,
    clip: {
