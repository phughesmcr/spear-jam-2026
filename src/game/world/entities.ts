import {
  ...
  type StoryEventId,
  type StoryTargetId,
} from "@/src/game/content/known_ids.ts";
import {
  ATTACK_PATTERN_AUTHORING_KEYS,
  ATTACK_TARGET_MODE_AUTHORING_KEYS,
  type AuthoringAttackDef,
} from "@/src/game/model/attack.ts";
import { coerceLookup, lowerFirst } from "@/src/engine/text.ts";
import { z } from "zod";
...
/** Authoring-side attack override; runtime codes live in {@link "@/src/game/model/attack.ts"}. */
export type AttackDef = AuthoringAttackDef;

export type { DialogueTreeId, DisplayName, ExamineTextId, StoryEventId, StoryTargetId };
/** Authoring enemy archetype key; runtime numeric codes live in {@link "@/src/game/content/enemies.ts"}. */
...
/** Authoring pickup kind strings; runtime ECS codes live in {@link "@/src/game/content/items.ts"}. */
