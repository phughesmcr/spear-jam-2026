import type { InputSize, PointerInput, PointerInteraction, PointerPhase } from "@/src/engine/input/mod.ts";
...
type PointerCallback = (input: PointerInput) => void;
type ElementRect = {
...
export function canvasPointerPosition(
  ...
  return {
    x: ((event.clientX - rect.left) / rect.width) * inputSize.width,
    y: ((event.clientY - rect.top) / rect.height) * inputSize.height,
  };
}
...
export function setupPointer(
  ...
  function handlePointerEvent(event: PointerEvent): void {
    ...
    const position = canvasPointerPosition(event, target.getBoundingClientRect(), inputSize());
    receiver({
      phase,
      x: position.x,
      y: position.y,
